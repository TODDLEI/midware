...
public class DatabaseConnectionManager {
    ...
    private static HashMap<String,DataSource> hikari
    ...
    public static void addHikariConnection(String name, String driver,
            String url, String user, String password, String database) throws HandlerException{
        
        initializeDriver(driver);
        
        if (hikari.containsKey(name)) {
            throw new HandlerException("Duplicate hikari connection: " + name);
        } else {
            ...
            //hc.addDataSourceProperty("databaseName", "usbn");
...
//        }
    ...
    private static Connection getHikariConnection(String name) throws HandlerException, SQLException {
        //return conns.get(name).newConnection();
        logger.dbConnection("Getting Hikari connection for "+name);
        DataSource dcw = hikari.get(name);
        if (dcw==null) {logger.midwareError("dcw is null");}
        roughCount++;
//        if (roughCount++ > SHUTDOWN_COUNT) {
//            roughCount = 1;
...
/**  Creates a new open database connection.  This connection will be private, and
    ...
    public static RegisteredConnection getNewHikariConnection(String name, String logDescription) throws HandlerException, SQLException {
        logger.dbConnection("Getting hikari connection for " + name);
        RegisteredConnection rc = new RegisteredConnection(getHikariConnection(name),
                name, logDescription,false);
        synchronized(openConnsLock) {
            openConns.get(name).add(rc);
        }
        logger.dbConnection("Returned a new hikari connection");
        //logger.debug(report());
        return rc;
    }
